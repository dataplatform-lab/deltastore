...
import org.apache.hadoop.fs.s3a.{S3AFileSystem, S3ClientFactory}
import org.apache.hadoop.fs.s3a.DefaultS3ClientFactory
import org.apache.hadoop.fs.s3a.S3AUtils.createAWSCredentialProviderSet
...
class S3FileSigner(
...
) extends CloudFileSigner {
  private val s3Client = ReflectionUtils
    .newInstance(classOf[DefaultS3ClientFactory], conf)
    .createS3Client(
      name,
      new S3ClientFactory.S3ClientCreationParameters()
        .withCredentialSet(createAWSCredentialProviderSet(name, conf))
        .withEndpoint(conf.get("fs.s3a.endpoint"))
        .withPathStyleAccess(
          conf.get("fs.s3a.path.style.access").equalsIgnoreCase("true")
        )
    );
  ...
  override def sign(path: Path): String = {
    ...
    val expiration =
      new Date(
        System.currentTimeMillis() + SECONDS.toMillis(
...
class AzureFileSigner(
    ...
    container: String,
    preSignedUrlTimeoutSeconds: Long,
    objectKeyExtractor: Path => String
) extends CloudFileSigner {
  ...
  private def getAccessPolicy: SharedAccessBlobPolicy = {
    ...
    sharedAccessPolicy.setPermissions(
      java.util.EnumSet.of(SharedAccessBlobPermissions.READ)
    )
  ...
  override def sign(path: Path): String = {
    ...
    val sasTokenCredentials = new StorageCredentialsSharedAccessSignature(
      sasToken
    )
...
object WasbFileSigner {
  private def getAccountFromAuthority(
      store: AzureNativeFileSystemStore,
      uri: URI
  ): String = {
  ...
  private def getContainerFromAuthority(
      store: AzureNativeFileSystemStore,
      uri: URI
  ): String = {
...
object AbfsFileSigner {
  private def getAbfsStore(
      fs: AzureBlobFileSystem
  ): AzureBlobFileSystemStore = {
    val getAbfsStoreMethod =
      classOf[AzureBlobFileSystem].getDeclaredMethod("getAbfsStore")
    getAbfsStoreMethod.setAccessible(true)
    getAbfsStoreMethod.invoke(fs).asInstanceOf[AzureBlobFileSystemStore]
  }

  private def getRelativePath(
      abfsStore: AzureBlobFileSystemStore,
      path: Path
  ): String = {
  ...
  def apply(
    ...
    val accountName =
      abfsConfiguration.accountConf("dummy").stripPrefix("dummy.")
