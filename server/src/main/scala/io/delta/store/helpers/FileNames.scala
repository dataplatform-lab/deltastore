...
object FileNames {
  ...
  val checkpointFilePattern =
    "\\d+\\.checkpoint(\\.\\d+\\.\\d+)?\\.parquet".r.pattern

  /* Returns the path for a given delta file. */
  def deltaFile(path: Path, version: Long): Path =
    new Path(path, f"$version%020d.json")
   ...
   *
   * Intended for use with listFrom to get all files from this version onwards.
   * The returned Path will not exist as a file.
   */
   ...
   *
   * In a future protocol version we will write this path instead of
   * checkpointFileSingular.
   *
   * Example of the format:
   * 00000000000000004915.checkpoint.0000000020.0000000060.parquet is
   * checkpoint part 20 out of 60 for the snapshot at version 4915. Zero
   * padding is for lexicographic sorting.
   */
  def checkpointFileWithParts(
      path: Path,
      version: Long,
      numParts: Int
  ): Seq[Path] = {
    Range(1, numParts + 1)
      .map(i =>
        new Path(
          path,
          f"$version%020d.checkpoint.$i%010d.$numParts%010d.parquet"
        )
      )
  }
  ...
  def isDeltaFile(path: Path): Boolean =
    deltaFilePattern.matcher(path.getName).matches()

  def checkpointVersion(path: Path): Long = path.getName.split("\\.")(0).toLong

  /* Get the version of the checkpoint, checksum or delta file. Throws an error
   * if an unexpected file type is seen. These unexpected files should be
   * filtered out to ensure forward compatibility in cases where new file types
   * are added, but without an explicit protocol upgrade.
   */
  ...
  /* Returns the `child` path as an absolute path and resolves any escaped char
   * sequences
   */
