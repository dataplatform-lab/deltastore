...
import io.delta.store.helpers.{DataTypeParser, JsonHelper}
import io.delta.store.internal.types.StructType

object DeltaActions {
  object Action {
    /* The maximum version of the protocol that this version of Delta
     * Standalone understands.
     */
    val readerVersion = 1
    val writerVersion = 2
    ...
    def fromJson(json: String): Action = {
      JsonHelper.fromJson[SingleAction](json).unwrap
    }
  }

  /* Represents a single change to the state of a Delta table. An order
   * sequence of actions can be replayed using [[InMemoryLogReplay]] to derive
   * the state of the table at a given point in time.
   */
  sealed trait Action {
    def wrap: SingleAction

    def json: String = JsonHelper.toJson(wrap)
  }
   ...
   * responsible for checking that they meet the minimum versions before
   * performing any other operations.
   *
   * Since this action allows us to explicitly block older clients in the case
   * of a breaking change to the protocol, clients should be tolerant of
   * messages and fields that they do not understand.
   */
  case class Protocol(
  ...
  object Protocol {
    ...
    def checkMetadataProtocolProperties(
        metadata: Metadata,
        protocol: Protocol
    ): Unit = {
      assert(
        !metadata.configuration.contains(MIN_READER_VERSION_PROP),
        s"Should not have the " +
  ...
  /* Sets the committed version for a given application. Used to make
   * operations like streaming append idempotent.
   */
  ...
  /* Adds a new file to the table. When multiple [[AddFile]] file actions are
   * seen with the same `path` only the metadata from the last one is kept.
   */
  case class AddFile(
      path: String,
  ...
  /* Logical removal of a given file from the reservoir. Acts as a tombstone
   * before a file is deleted permanently.
   *
   * Note that for protocol compatibility reasons, the fields
   * `partitionValues`, `size`, and `tags` are only present when the
   * extendedFileMetadata flag is true. New writers should generally be setting
   * this flag, but old writers (and FSCK) won't, so readers must check this
   * flag before attempting to consume those values.
   */
   ...
   * Non-CDC readers should ignore this, CDC readers should scan all
   * ChangeFiles in a version rather than computing changes from AddFile and
   * RemoveFile actions.
   */
  ...
  /* Updates the metadata of the table. Only the last update to the
   * [[Metadata]] of a table is kept. It is the responsibility of the writer to
   * ensure that any data already present in the table is still valid after any
   * change.
   */
  case class Metadata(
    ...
    lazy val dataSchema: StructType = {
      ...
      new StructType(
        schema.getFields.filterNot(f => partitions.contains(f.getName))
      )
    }
   ...
   * Commits can be referred to using a version and timestamp. The timestamp of
   * a commit comes from the remote storage `lastModifiedTime`, and can be
   * adjusted for clock skew. Hence we have the method `withTimestamp`.
   */
  trait CommitMarker {
   ...
   * Information stored in it is best effort (i.e. can be falsified by the
   * writer).
   */
  case class CommitInfo(
      // The commit version should be left unfilled during commit().
      // When reading a delta file,
      // we can infer the commit version from the file name and fill in this field then.
      @JsonDeserialize(contentAs = classOf[java.lang.Long])
      version: Option[Long],
      timestamp: Timestamp,
   ...
   *
   * With the inclusion of RemoveFile as an exposed Java API, and since it was
   * upgraded to match the latest Delta OSS release, we now had a case class
   * inside of [[SingleAction]] that had "primitive" default parameters. They
   * are primitive in the sense that Parquet4s would try to decode them using
   * the [[PrimitiveValueCodecs]] trait. But since these parameters have
   * default values, there is no guarantee that they will exist in the
   * underlying parquet checkpoint files. Thus (without these classes),
   * parquet4s would throw errors like this:
   *
   * Cause: java.lang.IllegalArgumentException: NullValue cannot be decoded to
   * required type at
   * com.github.mjakubowski84.parquet4s.RequiredValueCodec.decode(ValueCodec.scala:61)
   * at
   * com.github.mjakubowski84.parquet4s.RequiredValueCodec.decode$(ValueCodec.scala:58)
   * at
   * com.github.mjakubowski84.parquet4s.PrimitiveValueCodecs$$anon$5.decode(ValueCodec.scala:137)
   *
   * Note this only happens with "primitive" parameters with default arguments,
   * and not with "complex" or optional constructor parameters.
   *
   * We solve this issue by creating wrapper classes that wrap these primitive
   * constructor parameters in [[Option]]s, and then un-wrapping them as
   * needed, performing the appropriate Option[T] => T parameter conversions.
   */
  trait Parquet4sWrapper[T] {
    def unwrap: T
  }

  case class Parquet4sRemoveFileWrapper(
      path: String,
      @JsonDeserialize(contentAs = classOf[java.lang.Long])
      deletionTimestamp: Option[Long],
      ...
      size: Option[Long] = None,
      tags: Map[String, String] = null
  ) extends Parquet4sWrapper[RemoveFile] {
  ...
  case class Parquet4sSingleActionWrapper(
      txn: SetTransaction = null,
      add: AddFile = null,
      ...
      cdc: AddCDCFile = null,
      commitInfo: CommitInfo = null
  ) extends Parquet4sWrapper[SingleAction] {
