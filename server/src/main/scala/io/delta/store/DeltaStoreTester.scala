...
object DeltaStoreTester {
  private val parser = {
    val parser = ArgumentParsers
      .newFor("Delta Store Tester")
      .build()
      .defaultHelp(true)
      .description("Start the Delta Store Tester.")
    parser
    ...
    parser
      .addArgument("-p", "--pidfile")
      .required(true)
      .metavar("PIDFILE")
      .dest("pidfile")
      .help("The server pid file")
    parser
      .addArgument("-r", "--runtime")
      .required(true)
      .metavar("RUNTIME")
      .dest("runtime")
      .help("The server runtime")
    parser
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    val pidFile = ns.getString("pidfile")
    ...
    manager.register()
    manager.update()
    val server = buildAndStartServer(storeConfig)
    val runtime = ns.getString("runtime").toLong
