...
import io.delta.store.sources.StandaloneHadoopConf
...
trait LogStoreProvider {
  ...
  val acceptedLogStoreClassConfKeyRegex =
    f"((?:spark.)?${StandaloneHadoopConf.LOG_STORE_CLASS_KEY}|" +
      f"${StandaloneHadoopConf.LEGACY_LOG_STORE_CLASS_KEY})"
        .replace(""".""", """\.""")
  val acceptedLogStoreSchemeConfKeyRegex =
    """(?:spark\.)?delta\.logStore\.\w+\.impl"""
  ...
  def createLogStore(hadoopConf: Configuration): LogStore = {
    ...
    val logStoreClassName =
      hadoopConf.get(logStoreClassConfKey, defaultLogStoreClass)
    ...
    if (className == classOf[DelegatingLogStore].getName) {
      ...
      if (classOf[LogStore].isAssignableFrom(logStoreClass)) {
        ...
        throw new DeltaStandaloneException(
          s"Can't instantiate a LogStore with classname " +
            s"$className."
        )
      }
    }
   ...
   *   - For each config, check that the values are consistent across all
   *     accepted keys. Throw an error if they are not.
   *   - Set the "normalized" key to such value. This means future accesses can
   *     exclusively use the normalized keys.
   *
   * For scheme conf keys:
   *   - We accept 'delta.logStore.{scheme}.impl' and
  ...
  def normalizeHadoopConf(hadoopConf: Configuration): Unit = {
    ...
    val classConfs =
      hadoopConf.getValByRegex(acceptedLogStoreClassConfKeyRegex).asScala
  ...
  def checkLogStoreConfConflicts(hadoopConf: Configuration): Unit = {
    ...
    val schemeConf =
      hadoopConf.getValByRegex(acceptedLogStoreSchemeConfKeyRegex)
