...
from deltastore.reader import load_as_arrow
...
def test_predicates_filter(
    ...
    df = load_as_arrow(url, predicates=predicates)
    logger.info(df)
...
def test_predicates_limit(
    ...
    df = load_as_arrow(url, predicates=predicates, limit=limit)
    logger.info(df)
...
def test_projections_filter(
    ...
    df = load_as_arrow(url, predicates=predicates, columns=columns)
    logger.info(df)
